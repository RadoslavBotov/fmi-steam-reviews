...
import tqdm
from utility import class_rates, cm, read_csv, strip_list

def predict(x, threshold=0):
    if (x[8] - x[9] >= threshold):
        return "1"
...
for k in tqdm.tqdm(range(0, 100, 1)):
    for x in df_emotions:
        predictions.append(predict(x, - k / 100))
    acc = accuracy_score(df_class, predictions)
    if (acc > best_acc):
        best_acc = acc
...
predictions = []
for x in df_emotions:
    predictions.append(predict(x, threshold))
